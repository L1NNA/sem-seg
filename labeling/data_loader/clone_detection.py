...
class CloneDetection(Dataset):
    ...
    def __init__(self, config:Config, name):
        ...
        self.token_cache = join(config.data_path, 'cache',
            f'{name}_clone_detection_tokens.pt')
        self.indices_cache = join(config.data_path, 'cache',
            f'{name}_clone_detection_{range_len}_indices.pt')
    ...
    def __getitem__(self, index):
        ...
        x = torch.tensor([self.tokenizer.bos_token_id] + seg, dtype=torch.long)
        y = torch.tensor([self.tokenizer.bos_token_id] + clone, dtype=torch.long)
