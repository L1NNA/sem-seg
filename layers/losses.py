...
class SegInfoNCELoss(nn.Module):
    ...
    def forward(self, x_embeddings, y_embeddings):
        ...
        if self.n_windows > 1:
            ...
            x_embeddings = x_embeddings.reshape(batch_size, n_windows, -1)
            y_embeddings = y_embeddings.reshape(batch_size, n_windows, -1)
            ...
            similarities = F.cosine_similarity(x_embeddings.unsqueeze(1), \
                y_embeddings.unsqueeze(0), dim=2) / self.temperature
