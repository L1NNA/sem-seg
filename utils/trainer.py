    ...
    for epoch in range(init_epoch, config.epochs):
        ...
        for x, x_mask, segs, labels, y, y_mask in iterator:
            ...
            if config.do_ccr:
                ...
                if is_coe:
                    y_embed = model.module.embedding(y, y_mask) \
                        if isinstance(model, nn.DataParallel) \
                        else model.embedding(y, y_mask)
                else: 
...
def test(config:Config, model, test_loader, name, 
    ...
    for x, x_mask, segs, labels, y, y_mask in tqdm(test_loader, desc=name):
        ...
        if config.do_seg:
            ...
            preds = torch.argmax(probs, dim=1).cpu() # b x 1
        ...
        if config.do_cls:
            segs = segs.reshape(-1)
            cls_outputs = outputs[0] if is_coe else outputs
